...
zio_pop_transforms(zio_t *zio)
{
	...
	while ((zt = zio->io_transform_stack) != NULL) {
		...
		if (zt->zt_bufsize != 0) {
			if (ABD_IS_LINEAR(zio->io_data))
				zio_buf_free(ABD_TO_BUF(zio->io_data),
				    zt->zt_bufsize);
			else
...
zio_write(zio_t *pio, spa_t *spa, uint64_t txg, blkptr_t *bp,
    abd_t *data, uint64_t size, const zio_prop_t *zp,
    ...
    zio_priority_t priority, enum zio_flag flags, boolean_t labels)
{
	...
	if (zio_checksum_table[checksum].ci_eck) {
		 ...
		 */
		abd_t *wbuf;
		if (ABD_IS_LINEAR(data))
			wbuf = BUF_TO_ABD(zio_buf_alloc(size));
		else
			wbuf = abd_alloc(size);
...
zio_read_bp_init(zio_t *zio)
{
	...
	if (BP_GET_COMPRESS(bp) != ZIO_COMPRESS_OFF &&
		...
		if (ABD_IS_LINEAR(zio->io_data))
			cbuf = BUF_TO_ABD(zio_buf_alloc(psize));
		else
			cbuf = abd_alloc(psize);
	...
	if (compress != ZIO_COMPRESS_OFF) {
		...
		if (psize == 0 || psize == lsize) {
			...
			if (psize == lsize) {
				...
				if (ABD_IS_LINEAR(zio->io_data)) {
					cdata = BUF_TO_ABD(cbuf);
				} else {
					cdata = abd_alloc(lsize);
					abd_copy_from_buf(cdata, cbuf, psize);
					zio_buf_free(cbuf, lsize);
				}
    ...
    uint64_t offset)
{
	...
	if (gn != NULL) {
		zio = zio_rewrite(pio, pio->io_spa, pio->io_txg, bp,
		    BUF_TO_ABD(gn->gn_gbh), SPA_GANGBLOCKSIZE, NULL, NULL,
...
zio_gang_tree_assemble(zio_t *gio, blkptr_t *bp, zio_gang_node_t **gnpp)
{
	...
	zio_nowait(zio_read(gio, gio->io_spa, bp, BUF_TO_ABD(gn->gn_gbh),
...
zio_gang_tree_assemble_done(zio_t *zio)
{
	...
	ASSERT(zio->io_data == BUF_TO_ABD(gn->gn_gbh));
...
static void
zio_write_gang_member_done(zio_t *zio)
{
	abd_put_offset(zio->io_data);
}
...
zio_write_gang_block(zio_t *pio)
{
	 ...
	 */
	zio = zio_rewrite(pio, spa, txg, bp, BUF_TO_ABD(gbh), SPA_GANGBLOCKSIZE,
	...
	for (g = 0; resid != 0; resid -= lsize, g++) {
		...
		zio_nowait(zio_write(zio, spa, txg, &gbh->zg_blkptr[g],
		    ...
		    zio_write_gang_member_done, &gn->gn_child[g],
		    pio->io_priority, ZIO_GANG_CHILD_FLAGS(pio),
		    &pio->io_bookmark));
	}
...
zio_ddt_child_read_done(zio_t *zio)
{
	...
	if (zio->io_error == 0 && dde->dde_repair_data == NULL)
		dde->dde_repair_data = ABD_TO_BUF(zio->io_data);
	else
	...
	if (zio->io_child_error[ZIO_CHILD_DDT]) {
		...
		for (p = 0; p < DDT_PHYS_TYPES; p++, ddp++) {
			...
			zio_nowait(zio_read(zio, zio->io_spa, &blk,
			    BUF_TO_ABD(zio_buf_alloc(zio->io_size)),
...
zio_ddt_read_done(zio_t *zio)
{
	...
	if (zio->io_child_error[ZIO_CHILD_DDT]) {
		...
		if (dde->dde_repair_data != NULL) {
			abd_copy_from_buf(zio->io_data, dde->dde_repair_data,
			    zio->io_size);
...
zio_vdev_io_start(zio_t *zio)
{
	...
	if (!(zio->io_flags & ZIO_FLAG_PHYSICAL) &&
		...
		abd_t *abuf;
		if (ABD_IS_LINEAR(zio->io_data))
			abuf = BUF_TO_ABD(zio_buf_alloc(asize));
		else
			abuf = abd_alloc(asize);
	...
	if (zio->io_error == 0) {
		while (zio->io_cksum_report != NULL) {
			...
			if (asize != zio->io_size) {
				...
				abd_copy_to_buf(abuf, zio->io_data,
				    zio->io_size);
				bzero(abuf+zio->io_size, asize-zio->io_size);
			} else {
				abuf = abd_borrow_buf_copy(zio->io_data,
				    zio->io_size);
			}
			...
			if (asize != zio->io_size)
				...
				abd_return_buf(zio->io_data, abuf,
				    zio->io_size);
		}
	}
