...
static abd_t *abd_meta_alloc(size_t size)
{
	return (BUF_TO_ABD(zio_buf_alloc(size)));
}

static void abd_meta_free(abd_t *buf, size_t size)
{
	zio_buf_free(ABD_TO_BUF(buf), size);
}

static abd_t *abd_data_alloc(size_t size)
{
	return (abd_alloc(size));
}

static void abd_data_free(abd_t *buf, size_t size)
{
	abd_free(buf, size);
}
	...
	if (buf->b_data) {
		...
		if (!recycle) {
			if (type == ARC_BUFC_METADATA) {
				arc_buf_data_free(buf, abd_meta_free);
				arc_space_return(size, ARC_SPACE_META);
			} else {
				ASSERT(type == ARC_BUFC_DATA);
				arc_buf_data_free(buf, abd_data_free);
...
arc_get_data_buf(arc_buf_t *buf)
{
	...
	if (!arc_evict_needed(type)) {
		if (type == ARC_BUFC_METADATA) {
			buf->b_data = abd_meta_alloc(size);
			arc_space_consume(size, ARC_SPACE_META);
		} else {
			ASSERT(type == ARC_BUFC_DATA);
			buf->b_data = abd_data_alloc(size);
...
l2arc_compress_buf(l2arc_buf_hdr_t *l2hdr)
{
	...
	ddata = abd_borrow_buf_copy(l2hdr->b_tmp_cdata, l2hdr->b_asize);

	csize = zio_compress_data(ZIO_COMPRESS_LZ4, ddata,
	    cdata, l2hdr->b_asize);

	abd_return_buf(l2hdr->b_tmp_cdata, ddata, l2hdr->b_asize);
	...
	if (csize == 0) {
		...
		l2hdr->b_asize = csize;
		l2hdr->b_tmp_cdata = BUF_TO_ABD(cdata);
...
l2arc_release_cdata_buf(arc_buf_hdr_t *ab)
{
	...
	if (l2hdr->b_compress == ZIO_COMPRESS_LZ4) {
		 ...
		 */
		ASSERT(l2hdr->b_tmp_cdata != NULL);
		zio_data_buf_free(ABD_TO_BUF(l2hdr->b_tmp_cdata), ab->b_size);
	}
	l2hdr->b_tmp_cdata = NULL;
}
