...
dbuf_read_impl(dmu_buf_impl_t *db, zio_t *zio, uint32_t *flags)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		db->db.db_data = BUF_TO_ABD(zio_buf_alloc(DN_MAX_BONUSLEN));
		arc_space_consume(DN_MAX_BONUSLEN, ARC_SPACE_OTHER);
		if (bonuslen < DN_MAX_BONUSLEN)
			bzero(ABD_TO_BUF(db->db.db_data), DN_MAX_BONUSLEN);
		if (bonuslen)
			bcopy(DN_BONUS(dn->dn_phys),
			    ABD_TO_BUF(db->db.db_data), bonuslen);
		DB_DNODE_EXIT(db);
		dbuf_update_data(db);
		db->db_state = DB_CACHED;
		mutex_exit(&db->db_mtx);
...
dbuf_assign_arcbuf(dmu_buf_impl_t *db, arc_buf_t *buf, dmu_tx_t *tx)
{
	...
	if (db->db_state == DB_CACHED &&
		...
		mutex_exit(&db->db_mtx);
		(void) dbuf_dirty(db, tx);
		ASSERT_ABD_SCATTER(db->db.db_data);
		if (ABD_IS_LINEAR(buf->b_data))
			abd_copy_from_buf(db->db.db_data,
			    ABD_TO_BUF(buf->b_data), db->db.db_size);
		else
...
dbuf_clear(dmu_buf_impl_t *db)
{
	...
	if (db->db_state == DB_CACHED) {
		ASSERT(db->db.db_data != NULL);
		if (db->db_blkid == DMU_BONUS_BLKID) {
			zio_buf_free(ABD_TO_BUF(db->db.db_data),
			    DN_MAX_BONUSLEN);
